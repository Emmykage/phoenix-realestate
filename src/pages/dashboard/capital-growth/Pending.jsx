...
const CapitalTransactions = () => {
  ...
  return (
    <div class="block -w-full py-2 align-middle">
      ...
      <table class="w-full bg-white border border-gray-200 rounded-md border-separate border-spacing-0 table-auto overflow-hidden">
        ...
        {loading ? (<td colSpan={6}><Loader/></td>) :
          capitalTransactions?.length < 1 ? <td colSpan={6} className='py-6 font-medium text-center'> No Transaction Available </td> : 
          capitalTransactions?.map(transaction => (
              ...
              <td className="text-blue-900 pL-4">{dateFormater(transaction.created_at)} </td>
                 <td className={`${transaction?.status === "completed" ? "text-green-800" : " text-orange-700"} font-medium px-4`}>{transaction.status}</td>
            </tr>
            ))
