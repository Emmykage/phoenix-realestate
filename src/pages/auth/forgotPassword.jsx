...
import { userConfirmation, userResetToken, userSession } from '../../redux/actions/auth'
...
const ForgotPassword = () => {
    ...
    const handleFormSubmit = (e) => {
       e.preventDefault();
               ...
               dispatch(userResetToken(formInput)).then(result => {
                   if(userResetToken.fulfilled.match(result)){
                       
                       toast(result.payload?.message || "Password Sent", {type: "success"})

                       dispatch(SET_LOADER(false))
                        navigate(`/auth/password-reset?email=${formInput.email}`);
  ...
  return (
    <div>
        <SubHeader tittle={"Forgot Password Account"} link={"/"}  />
